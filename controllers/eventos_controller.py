...
def eventos_nuevo():
    if request.method == "POST":
        ...
        if data["modalidad"] == "virtual":
            if not data["enlace_virtual"]:
                errores.append("Para eventos virtuales, el enlace es obligatorio.")
            # Limpiar campos de ubicación para eventos virtuales
            data["lugar"] = ""
            data["ciudad"] = ""
        else:
            if not data["lugar"]:
                errores.append("Para eventos presenciales, el lugar es obligatorio.")
            if not data["ciudad"]:
                errores.append("Para eventos presenciales, la ciudad es obligatoria.")
            # Limpiar enlace para eventos presenciales
            data["enlace_virtual"] = ""
        ...
        if errores:
            ...
            return render_template("events/evento_form.html", modo="nuevo", tipos=tipos, ev=data)
    ...
    if request.method == "POST":
        ...
        if errores:
            for error in errores:
                flash(error, "danger")
            tipos = ['foro','debate','taller','simposio','panel de expertos','seminario','conferencia']
            return render_template("events/evento_form.html", modo="editar", tipos=tipos, ev=data)
...
def escanear_qr_asistencia(eid, token):
    """Procesa el escaneo del QR y marca asistencia automáticamente - SIN VALIDACIÓN DE HORARIO"""
    ev = MEvento.obtener(eid)
    if not ev:
    ...
    # ELIMINADA LA VALIDACIÓN DE HORARIO - Ahora funciona en cualquier momento
